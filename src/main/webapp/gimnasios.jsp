                    ...
                    <div class="card bg-black border-secondary text-white h-100 shadow-lg d-flex flex-column" style="border-radius: 15px; overflow: hidden;">
                        ...
                        <div class="p-4 d-flex flex-column flex-grow-1">
                            ...
                            <div class="d-flex flex-wrap gap-2 mt-3 mb-3">
                                <c:set var="sedeStr" value="<%= s.getBeneficios() %>" />
                                
                                <c:choose>
                                    <c:when test="${not empty sedeStr and sedeStr != 'null' and sedeStr != ''}">
                                        <c:forEach var="ben" items="${fn:split(sedeStr, ',')}">
                                            <c:set var="b" value="${fn:trim(ben)}" />
                                            <span class="badge bg-dark border border-secondary text-light p-1 px-2" style="font-size: 0.8rem;">
                                                <c:choose>
                                                    <c:when test="${fn:containsIgnoreCase(b, 'Piscina')}"><i class="fa-solid fa-person-swimming text-info me-1"></i></c:when>
                                                    <c:when test="${fn:containsIgnoreCase(b, 'Sauna')}"><i class="fa-solid fa-hot-tub-person text-danger me-1"></i></c:when>
                                                    <c:when test="${fn:containsIgnoreCase(b, 'Kids')}"><i class="fa-solid fa-child-reaching text-warning me-1"></i></c:when>
                                                    <c:when test="${fn:containsIgnoreCase(b, 'Wifi')}"><i class="fa-solid fa-wifi text-primary me-1"></i></c:when>
                                                    <c:when test="${fn:containsIgnoreCase(b, 'Parking') || fn:containsIgnoreCase(b, 'Estacionamiento')}"><i class="fa-solid fa-square-parking text-secondary me-1"></i></c:when>
                                                    <c:when test="${fn:containsIgnoreCase(b, 'Ducha')}"><i class="fa-solid fa-shower text-info me-1"></i></c:when>
                                                    <c:when test="${fn:containsIgnoreCase(b, 'Cardio')}"><i class="fa-solid fa-heart-pulse text-danger me-1"></i></c:when>
                                                    <c:when test="${fn:containsIgnoreCase(b, 'Peso') || fn:containsIgnoreCase(b, 'Fuerza')}"><i class="fa-solid fa-dumbbell text-secondary me-1"></i></c:when>
                                                    <c:when test="${fn:containsIgnoreCase(b, 'Baile') || fn:containsIgnoreCase(b, 'Zumba')}"><i class="fa-solid fa-music text-success me-1"></i></c:when>
                                                    <c:when test="${fn:containsIgnoreCase(b, 'Spinning')}"><i class="fa-solid fa-person-biking text-warning me-1"></i></c:when>
                                                    <c:otherwise><i class="fa-solid fa-check text-success me-1"></i></c:otherwise>
                                                </c:choose>
                                                ${b}
                                            </span>
                                        </c:forEach>
                                    ...
                                    <c:otherwise>
                                        <span class="text-secondary small"><i class="fa-solid fa-circle-info me-1"></i> Beneficios no especificados.</span>
                                    </c:otherwise>
                                </c:choose>
                            </div>
                            ...
                            <div class="pt-3 border-top border-secondary d-flex justify-content-between align-items-center">
                                ...
                                <a href="${pageContext.request.contextPath}/SedeDetalle?id=<%= s.getIdSede()%>"
                                   class="btn btn-sm btn-outline-warning fw-bold px-3"
